...
public class PDFPageCounter {
    ...
    private static void createAndShowGUI() throws URISyntaxException {
        ...
        String jarPath = PDFPageCounter.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
    ...
    private static void procurarePlotarArquivos (JTextArea textArea, String jarDir) {
            File rootFolder = new File(jarDir);
            processFolder(rootFolder, textArea);
            SwingUtilities.invokeLater(() -> textArea.append("\n\nTotal de arquivos " + arquivosTotais +"\n"));
            SwingUtilities.invokeLater(() -> textArea.append("Total de paginas " + paginas +"\n"));
            BigDecimal valorArredondado = BigDecimal.valueOf(paginas/7).setScale(2, RoundingMode.HALF_UP);
            SwingUtilities.invokeLater(() -> textArea.append("Devem ser estudadas " + valorArredondado + " paginas por dia, considerados 7 dias por semana.\n"));
    }
    ...
    private static void processPDF(File file, JTextArea textArea) {
        try (PDDocument document = PDDocument.load(file)) {
            ...
            arquivosTotais++;
            SwingUtilities.invokeLater(() -> 
                textArea.append(file.getAbsolutePath() + " tem " + pageCount + " paginas.\n"));            
        } catch (IOException e) {
            e.printStackTrace();
        }
